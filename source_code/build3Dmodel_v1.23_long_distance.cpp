...
int Check_File_Existence(string &file)
{
	ifstream fin;
	string buf;
	...
	if(fin.fail()!=0)
	{
		fprintf(stderr,"result file not found [%s] !!!\n",file.c_str());
		return -1;
	}
	return 1;
}
...
void WS_Pair_Modeler_Python_Write(FILE *fp,const char *nam1,const char *nam2,string &alifile,string &pdb_root,int MOL_NUM=1)
{
	...
	//final items
	fprintf(fp,"a.max_ca_ca_distance = 30          # long distance restraints\n");
...
//	return;
	...
	//if failed, then rename '.ini' to '.pdb'
	sprintf(wscommand,"%s.B99990001.pdb",ssstemp.c_str());
	string reso_file=wscommand;
	if(Check_File_Existence(reso_file)==-1)
	{
		sprintf(wscommand,"cp %s.ini %s.B99990001.pdb",
			ssstemp.c_str(),ssstemp.c_str());
		system(wscommand);
	}
